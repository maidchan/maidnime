...
const image = computed(
  () => props.anime?.image || "https://images2.alphacoders.com/131/1313627.jpg"
);
const description = computed(() =>
  props.anime?.last_episode
    ? `Episode Terbaru: ${props.anime.last_episode}`
    : "Streaming anime subtitle Indonesia gratis tanpa iklan."
);
...
const watchNow = async () => {
  try {
    ...
    if (props.anime?.id) {
      ...
      if (
        typeof props.anime.id === "string" &&
        props.anime.id.includes("episode")
      ) {
...
<template>
  <div class="relative w-full h-[500px] md:h-[600px] overflow-hidden group">
    ...
    <!-- Gradient Overlay -->
    <div
      class="absolute inset-0 bg-linear-to-t from-background-dark via-background-dark/60 to-transparent"
    ></div>
    <div
      class="absolute inset-0 bg-linear-to-r from-background-dark via-background-dark/40 to-transparent"
    ></div>
    ...
    >
      <div class="max-w-2xl flex flex-col gap-4 md:gap-6 mt-16 md:mt-20">
        ...
        <h1
          class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-tight tracking-tight text-shadow-sm"
          v-html="title.replace(':', ':<br/>')"
        ></h1>

        <p
          class="text-gray-300 text-base md:text-lg line-clamp-2 md:line-clamp-3 max-w-xl text-shadow-sm"
        >
          {{ description }}
        </p>

        <div
          class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4 pt-2"
        >
          <button
            @click="watchNow"
            class="flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-white px-6 md:px-8 py-3 rounded-lg font-bold transition-transform active:scale-95 shadow-lg shadow-primary/25 min-h-[44px]"
          >
          ...
          >
            <span
              ...
              :class="{ 'fill-1': isInList }"
              >{{ isInList ? "done" : "add" }}</span
            >
